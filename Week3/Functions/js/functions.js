...
function dniLetter( dni ) { if(dni < 0 || isNaN(dni)){return 'Invalid parameter';}
...
function findLongestWord( array_words ){
	...
	var longitud = array_words.length;
	var count = 0
...
array_lengths.push( findLongestWord( ["Dog", "Cat", "Elephant", "Snake"] ) );

console.log( "These are the longest lengths of each group: " + array_lengths );
console.log( calculateAverage( array_lengths ) === 6.5 && "The average of longest lengths is: " + calculateAverage( array_lengths ))

function calculateAverage( array )
{ var sum = 0;
	for (var array_lengths = 0, length = array.length; array_lengths < length; array_lengths++ ) {
		sum += array[array_lengths];
	}
	return sum/length ;
}
